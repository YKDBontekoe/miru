...
**Note on Prompt:** Jules is instructed to strictly follow project architecture (Domain logic in `backend/app/domain/`, routes in `backend/app/api/v1/`, frontend in `frontend/app/`), code style (100-char lines, type hints, specific import orders, `str | None` not `Optional[str]`), and test requirements (mock all external services including Supabase, OpenRouter, Neo4j in tests — never call real APIs).
