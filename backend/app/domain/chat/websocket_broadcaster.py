...
class ChatWebSocketBroadcaster:
    ...
    @staticmethod
    def parse_transcript(result: Any, agent_names: list[str]) -> list[tuple[str, str]]:
        """Convert a CrewAIResponse object into (agent_name, message) pairs."""
        from app.domain.chat.crew_orchestrator import CrewAIResponse

        if not isinstance(result, CrewAIResponse):
            # Fallback if somehow passed a string
        ...
        for seg in result.segments:
            if not seg.message.strip():
                continue

            # Use the agent name only if it matches one of the known agents,
            # otherwise fall back to empty string to pick the default/sole agent.
            final_name = seg.agent_name if seg.agent_name.lower() in name_set else ""
            parsed_segments.append((final_name, seg.message.strip()))

        return parsed_segments if parsed_segments else [("", "")]
    ...
    async def persist_and_broadcast_agent_response(
        ...
        segments = self.parse_transcript(result, agent_names)
