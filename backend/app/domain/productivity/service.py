...
@asynccontextmanager
async def _handle_db_errors(operation_name: str) -> AsyncGenerator[None, None]:
    """Handle database errors consistently across productivity services."""
    try:
        yield
    except (IntegrityError, OperationalError, DBConnectionError, ValueError) as e:
        logger.exception(f"Failed to {operation_name}")
        raise HTTPException(
            status_code=500, detail=f"Database error occurred while {operation_name}"
        ) from e
    except Exception as e:
        logger.exception(f"Unexpected error in {operation_name}")
        raise HTTPException(status_code=500, detail=f"Failed to {operation_name}") from e
