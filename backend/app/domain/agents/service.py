...
class AgentService:
    ...
    async def update_agent(
        ...
        if new_capability_ids is not None:
            await self.repo.update_capabilities(agent, new_capability_ids)
            effective_cap_ids = new_capability_ids
        else:
            effective_cap_ids = await self.repo.get_capability_ids(agent)
        ...
        if new_integration_ids is not None:
            await self.repo.update_integrations(agent, new_integration_ids, new_integration_configs)
