...
def _build_agent_response(agent: AgentEntity) -> AgentResponse:
    ...
    return AgentResponse(
        ...
        capabilities=agent.capability_ids,
        integrations=agent.integration_ids,
...
class AgentService:
    ...
    async def create_agent(self, agent_data: AgentCreate, user_id: UUID) -> AgentResponse:
        ...
        if agent_data.integrations:
            for integration_id in agent_data.integrations:
                config = agent_data.integration_configs.get(str(integration_id), {})
                integrations_data.append({"id": integration_id, "config": config})

        agent_entity = await self.repo.create_agent(
            user_id=user_id,
            name=agent_data.name,
            personality=agent_data.personality,
            description=agent_data.description,
        ...
        return _build_agent_response(agent_entity)
    ...
    async def update_agent(
        ...
        effective_cap_ids = (
            new_capability_ids if new_capability_ids is not None else agent.capability_ids
        )
        ...
        if new_integration_ids is not None:
            ...
            for integration_id in new_integration_ids:
                config = new_integration_configs.get(str(integration_id), {})
                integrations_data.append({"id": integration_id, "config": config})
        ...
        updated_entity = await self.repo.update_agent(
            agent_id=agent_id,
            user_id=user_id,
            fields=fields,
            new_capability_ids=new_capability_ids,
            new_integrations_data=integrations_data,
        )
        if not updated_entity:
            return None
        return _build_agent_response(updated_entity)
    ...
    async def list_templates(self, skip: int = 0, limit: int = 100) -> list[AgentTemplateResponse]:
        ...
        # Assuming AgentTemplateResponse can validate from the dict representation of entity
        return [AgentTemplateResponse.model_validate(t.__dict__) for t in templates]
