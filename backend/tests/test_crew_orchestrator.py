...
import asyncio
from app.domain.chat.crew_orchestrator import CrewOrchestrator, SingleAgentResponse, MultiAgentResponse, AgentMessage
import uuid
...
async def test_execute_crew_task_single_agent_success(mock_agent):
    with patch("app.domain.chat.crew_orchestrator.Crew") as mock_crew_class:
        ...
        result = await CrewOrchestrator.execute_crew_task(
            room_agents=[mock_agent],
            user_message="Hello",
            user_id=uuid.uuid4()
        )
        assert '{"message":"Success"}' == result.replace(" ", "")
...
async def test_execute_crew_task_multi_agent_success(mock_agent):
    ...
    with patch("app.domain.chat.crew_orchestrator.Crew") as mock_crew_class:
        mock_crew_instance = mock_crew_class.return_value
        mock_result = MockOutput(pydantic_obj=MultiAgentResponse(messages=[AgentMessage(agent_name="Test Agent", message="Success")]))
        ...
        result = await CrewOrchestrator.execute_crew_task(
            room_agents=[mock_agent, mock_agent2],
            user_message="Hello",
            user_id=uuid.uuid4()
        )
        assert '{"messages":[{"agent_name":"TestAgent","message":"Success"}]}' == result.replace(" ", "")

@pytest.mark.asyncio
async def test_execute_crew_task_retry(mock_agent):
    with patch("app.domain.chat.crew_orchestrator.Crew") as mock_crew_class, \
         patch("app.domain.chat.crew_orchestrator.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
