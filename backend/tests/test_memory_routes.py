...
def test_get_memory_graph_success(client: TestClient) -> None:
    user_id = uuid4()
    mock_service = MagicMock()

    mock_graph_data = {"nodes": [{"id": "node-1"}], "edges": []}
    mock_service.get_memory_graph = AsyncMock(return_value=mock_graph_data)
