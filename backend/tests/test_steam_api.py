...
def mock_settings() -> typing.Generator[typing.Any, None, None]:
    with patch("app.infrastructure.external.steam_client.get_settings") as mock:
...
@pytest.mark.asyncio
async def test_get_player_summaries(mock_settings: typing.Any, steam_client: SteamClient) -> None:
...
@pytest.mark.asyncio
async def test_get_owned_games(mock_settings: typing.Any, steam_client: SteamClient) -> None:
    steam_id = "76561197960435530"
    mock_data = {
        "response": {
            "game_count": 2,
...
@pytest.mark.asyncio
async def test_get_player_summaries_no_key(steam_client: SteamClient) -> None:
    with patch("app.infrastructure.external.steam_client.get_settings") as mock:
...
@pytest.mark.asyncio
async def test_get_owned_games_no_key(steam_client: SteamClient) -> None:
    with patch("app.infrastructure.external.steam_client.get_settings") as mock:
        mock.return_value.steam_api_key = None
        games = await steam_client.get_owned_games("76561197960435530")
        assert games == []
...
@pytest.mark.asyncio
async def test_resolve_vanity_url_no_key(steam_client: SteamClient) -> None:
    with patch("app.infrastructure.external.steam_client.get_settings") as mock:
        mock.return_value.steam_api_key = None
