...
async def test_spotify_currently_playing_errors(token: str) -> None:
    # missing item
    with patch(
        "app.infrastructure.external.spotify._get_async", return_value={"is_playing": False}
    ):
    ...
    # RequestError
    with patch(
        "app.infrastructure.external.spotify._get_async", side_effect=httpx.RequestError("error")
    ):
        res = await get_currently_playing(token)
        assert res is None
...
async def test_discord_server_info_errors(token: str) -> None:
    ...
    # RequestError
    with patch(
        "app.infrastructure.external.discord._get_async", side_effect=httpx.RequestError("error")
    ):
        res = await get_server_info(token, "1")
        assert res is None
...
async def test_discord_send_message_errors(token: str) -> None:
    # RequestError
    with patch(
        "app.infrastructure.external.discord._post_async", side_effect=httpx.RequestError("error")
    ):
...
async def test_discord_get_async_status_error(token: str) -> None:
    # HTTPStatusError
    with patch(
        "app.infrastructure.external.discord._get_async",
...
async def test_discord_post_async_status_error(token: str) -> None:
    # HTTPStatusError
    with patch(
        "app.infrastructure.external.discord._post_async",
        side_effect=httpx.HTTPStatusError(
            "error",
...
async def test_spotify_get_async_status_error(token: str) -> None:
    # HTTPStatusError
    with patch(
        "app.infrastructure.external.spotify._get_async",
        side_effect=httpx.HTTPStatusError(
            "error",
            request=httpx.Request("GET", "url"),
            response=httpx.Response(400, request=httpx.Request("GET", "url")),
