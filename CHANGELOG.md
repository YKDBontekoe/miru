...
- **Components**: Added JSDoc blocks to `AgentAvatar` and `BackendSplash` components, improved the `useAgentStore` hook description to clarify its optimistic update strategies, and added JSDoc comments to `useChatRoomSetup` and `useDebounce` describing their setup/debounce behavior and side effects.
