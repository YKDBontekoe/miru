...
- **Components**: Added JSDoc blocks to `AgentAvatar` and `BackendSplash` components, improved the `useAgentStore` hook description, and added JSDoc comments to `useChatRoomSetup` and `useDebounce` hooks to clarify their optimistic update strategies and side effects.
