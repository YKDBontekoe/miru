...
const renderAgentItem = ({ item, extraData }: { item: Agent | { id: 'all'; isAll: true }; extraData: any }) => {
  const { selectedAgentId, onSelectAgent, t } = extraData;
  if ('isAll' in item) {
    return (
      <ScalePressable
      ...
      >
        <AppText variant="caption" className={`font-bold ${selectedAgentId ? 'text-[#5A7467]' : 'text-[#147D64]'}`}>
...
export const ChatListHeader = React.memo(function ChatListHeader({
  ...
  const agentItems = useMemo(() => [{ id: 'all', isAll: true } as const, ...agents], [agents]);

  const agentExtraData = useMemo(() => ({ selectedAgentId, onSelectAgent, t }), [selectedAgentId, onSelectAgent, t]);
      ...
      {agents.length > 0 ? (
        <View className="bg-white rounded-3xl border border-[#DDE8E0] py-[14px] mb-3 shadow-md">
          ...
          <FlatList
            ...
            data={agentItems}
            renderItem={(props) => renderAgentItem({ ...props, extraData: agentExtraData })}
            keyExtractor={(item) => item.id}
            extraData={agentExtraData}
          />
        </View>
      ) : null}
