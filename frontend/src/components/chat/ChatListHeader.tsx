...
export const ChatListHeader = React.memo(function ChatListHeader({
  ...
  const filterItems: FilterItemType[] = useMemo(() => [
  ...
  const renderFilterItem = useCallback(
    ({ item }: ListRenderItemInfo<FilterItemType>) => {
      if (item.type === 'sort') {
  ...
  const filterKeyExtractor = useCallback((item: FilterItemType) => item.label, []);
  ...
  const agentPillKeyExtractor = useCallback((item: Agent) => item.id, []);

  const agentsListHeader = useMemo(
    () => (
      <ScalePressable
  ...
  return (
    <>
      ...
      <View className="bg-white rounded-3xl border border-[#DDE8E0] py-[14px] mb-3 shadow-md">
        <View className="px-[14px] flex-row items-center rounded-[14px] border border-[#DDE8E0] bg-[#ECF5F0] px-2.5 mb-2.5 mx-[14px]">
