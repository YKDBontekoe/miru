...
type FilterOption =
  ...
  | { type: 'toggle'; id: string; label: string; active: boolean; onToggle: () => void };

export const ChatListHeader = React.memo(function ChatListHeaderComponent({
  t,
  query,
  onChangeQuery,
  sortMode,
  ...
  const filterOptionsData = useMemo<FilterOption[]>(
    () => [
  ...
  const renderFilterOption = useCallback(
    ({ item }: { item: FilterOption }) => {
      if (item.type === 'sort') {
        ...
        return (
          <ScalePressable
            onPress={() => onChangeSortMode(item.id as SortMode)}
