...
export function RoomPromptRail({
  ...
  const renderContextItem = useCallback(({ item: value }: { item: string }) => (
    <Pressable
  ...
  const extraDataPrompts = useMemo(() => ({ isStreaming, canSave }), [isStreaming, canSave]);
