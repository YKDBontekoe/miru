...
export const RoomPromptRail = React.memo(function RoomPromptRailComponent({
  prompts,
  isStreaming,
  saveLabel,
  heading,
  ...
  const extractPromptKey = useCallback((item: PromptItem) => item.id, []);
  const extractContextKey = useCallback((item: string) => item, []);
      ...
      <View className="rounded-[18px] border border-[#DDE8E0] bg-white py-2 shadow-md">
        ...
        {contextActions && contextActions.length > 0 && onContextPress ? (
          <FlatList
            ...
            keyExtractor={extractContextKey}
